...
func AreEqualFields(flds1, flds2 ast.FieldList) bool {
	...
	for _, fld := range flds2 {
		...
		s1.WriteString(genArgsDefnString(fld.Arguments) + ": ")
		s1.WriteString(fld.Type.String() + "\n")

		s2.WriteString("\t" + val.Name)
		s2.WriteString(genArgsDefnString(val.Arguments) + ": ")
...
func genArgsDefnString(args ast.ArgumentDefinitionList) string {
	if args == nil {
		return ""
	}
...
func genArgumentsString(args ast.ArgumentList) string {
	...
	sch.WriteString("(")
	for _, arg := range args {
		argStrs = append(argStrs, arg.Name+": \""+arg.Value.Raw+"\"")
	}

	sort.Slice(argStrs, func(i, j int) bool { return argStrs[i] < argStrs[j] })
	sch.WriteString(strings.Join(argStrs, ",") + ")")
...
func genFieldString(fld *ast.FieldDefinition) string {
	...
	sch.WriteString(genArgsDefnString(fld.Arguments) + ": ")
	sch.WriteString(fld.Type.String() + " ")
	sch.WriteString(genDirectivesString(fld.Directives) + "\n")
